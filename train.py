...
l_unlabeled_dataset = data.EvalDataset('wav8k/train', 'mix', batch_size = 1)# на самом деле он labeled, но мы берем только mix, без s1 s2
...
for epoch in range(EPOCHS):
    ...
    print('Epoch {0} | Iter {1} | Average Loss {2:.3f} | '
          'Current Loss {3:.6f} | {4:.1f} ms/batch'.format(epoch + 1, i + 1, total_correctness_loss / (i + 1),
           np.array(correctness_loss.item()), 1000 * (time.time() - start) / (i + 1)), flush=True)
    ...
    for i, (data) in enumerate(tqdm(unlabeled_loader)):
        ...
        #"-------BREAKDOWN----------"
        flat_estimate = train_utils.remove_pad_and_flat(teacher_pred.data, mixture_lengths) #каким-то образом разделяем выход сети
    ...
    LOSS = 1/len(labeled_loader) * total_correctness_loss + np.exp(epoch+1/EPOCHS-1)/(len(labeled_loader)+len(unlabeled_loader))*total_consist_loss
    #if not cross_valid:
    student_optimizer.zero_grad()
    LOSS.backward()
    torch.nn.utils.clip_grad_norm_(student_model.parameters(),
